...
request.open('GET', 'http://localhost:7000/api/type-barang/all', true);
request.onload = function () {
  ...
  if (request.status >= 200 && request.status < 400) {
      ...
      h1.textContent = `Id Type : ${getDataJsonBody.idType}`;

      const h2 = document.createElement('h2');
      h2.textContent = `Tipe Barang : ${getDataJsonBody.tipeBarang}`;

      const h3 = document.createElement('h2');
      h3.textContent = `Jenis Barang : ${getDataJsonBody.jenisBarang}`;
