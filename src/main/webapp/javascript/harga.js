...
request.open('GET', 'http://localhost:7000/api/harga/all', true);
request.onload = function () {
  ...
  if (request.status >= 200 && request.status < 400) {
      ...
      h1.textContent = `Id Harga : ${getDataJsonBody.idHarga}`;

      const h2 = document.createElement('h2');
      h2.textContent = `Harga Satuan : ${getDataJsonBody.hargaSatuan}`;

      const h3 = document.createElement('h2');
      h3.textContent = `Discount : ${getDataJsonBody.discount} %`;
